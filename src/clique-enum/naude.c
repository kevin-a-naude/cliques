...
int main(int argc, const char* argv[])
{
    if (argc < 2)
    {
        printf("Enumerates cliques by Naude's algorithm.\n");
        printf("\n");
        printf("Usage:\n");
    ...
    else
    {
        ...
        FILE* fout;
        clique_context_t cc;
        clique_context_naude(&cc);

        fout = stdout;

        if (write_headers)
        {
            fprintf(fout, "algorithm,filename,num-vertices,num-edges,deg-variance,max-clique-size,num-rec-calls,num-cliques,seconds\n");
            fflush(fout);
        }
